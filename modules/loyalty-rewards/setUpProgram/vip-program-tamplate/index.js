...
function VIPProgramTamplate() {
  ...
  const [tierModalKey, setTierModalKey] = useState(0);
  ...
  return (
    <>
      ...
      >
        <AitPageHeader
          hideButton={parseFloat(vipProgramData?.lyt_milestone_setting) === 0}
          disabled={pageLoading}
          buttonLabel="Create"
          onButtonClick={() => {
            if (vipProgramData?.lyt_milestone_setting?.hasOwnProperty('id')) {
              setRowData(undefined);
              setTierCreateModal(true);
              setTierModalKey((prev) => prev + 1);
            } else {
        ...
        />
        <AitConfirmationModal
          visible={programConfModal}
          setVisible={() => {
            setProgramconModal(false);
          }}
          confirmText="Ok"
          message="VIP program configuration can not be modified"
          description={`As there are current customer(s) in your loyalty tier, you are
                  unable to modify the configuration. To proceed with updating
                  the configuration, it's necessary to first remove all
                  customers from the loyalty tier. Kindly reach out to our
                  support team to assist you in removing the customer(s) from
                  the loyalty tier.`}
          onConfirm={() => {
            setProgramconModal(false);
          }}
        ...
        <AitModal
          ...
          title={
            <>
              <AitText strong size={22} bottommargin={5}>
                {!rowData?.id ? 'Create new tier' : 'Edit customer tier'}
              </AitText>

              {/* {!rowData?.id && (
                <AitText size={13} type="secondary">
        ...
        >
          ...
          <CreateTierTamplate
            key={
              rowData?.id
                ? `edit-tier-${rowData?.id}`
                : `create-tier-${tierModalKey}`
            }
            form={form}
