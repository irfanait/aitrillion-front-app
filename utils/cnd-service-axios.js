...
const cndServiceApi = axios.create({
  baseURL: process.env.NEXT_PUBLIC_CND_API_URL,
  headers: {
...
// 🔐 Request interceptor
cndServiceApi.interceptors.request.use((config) => {
...
// 🛑 Response interceptorcndServiceApi.interceptors.response.use(
cndServiceApi.interceptors.response.use(
  (response) => {
    if (
